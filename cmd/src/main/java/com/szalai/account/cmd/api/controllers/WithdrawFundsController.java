package com.szalai.account.cmd.api.controllers;

import com.szalai.account.cmd.api.commands.DepositFundsCommand;
import com.szalai.account.cmd.api.commands.WithdrawFundsCommand;
    ...
    public ResponseEntity<BaseResponse> withdrawFunds(
        ...
        try{
            ...
            return new ResponseEntity<>(new BaseResponse(
                    "Bank account withdraw funds request completed successfully"
            ), HttpStatus.OK);
        } catch (IllegalStateException | AggregateNotFoundException e){
            logger.log(Level.WARNING, MessageFormat.format("Client made a bad request - {0}.", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e){
            String safeErrorMessage = MessageFormat.format("Error while processing request to withdraw funds from bank account for id - {0}", id);
