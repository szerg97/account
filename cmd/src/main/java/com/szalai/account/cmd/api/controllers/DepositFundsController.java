...
import com.szalai.account.cmd.api.dto.OpenAccountResponse;
    ...
    public ResponseEntity<BaseResponse> depositFunds(
        ...
        try{
            ...
            return new ResponseEntity<>(new BaseResponse(
                    "Bank account deposit funds request completed successfully"
            ), HttpStatus.OK);
        } catch (IllegalStateException | AggregateNotFoundException e){
            logger.log(Level.WARNING, MessageFormat.format("Client made a bad request - {0}.", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e){
            String safeErrorMessage = MessageFormat.format("Error while processing request to deposit funds to bank account for id - {0}", id);
